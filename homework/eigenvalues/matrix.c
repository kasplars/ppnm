...
void jacobi_diag(gsl_matrix * A,gsl_matrix * V) {	// A becomes the matrix with eigenvalues 
							// and V is the matrix which will contain eigenvectors
...
void jacobi_diag_optim(gsl_matrix * A,gsl_matrix * V) {	// A becomes the matrix with eigenvalues 
							// and V is the matrix which will contain eigenvectors
	int n = A->size1; gsl_matrix_set_identity(V);
	// double threshold = 10000*LDBL_EPSILON;
	double threshold = 1000000000*LDBL_EPSILON;
	do {
		for(int p=0;p<n-1;p++) {
			for(int q=p+1;q<n;q++){
			double apq=gsl_matrix_get(A,p,q);
			...
			// update elements on and above diagonal according to eq. (10):
			double theta=0.5*atan2(2*apq,aqq-app);
			double c=cos(theta),s=sin(theta);
			double new_app=c*c*app-2*s*c*apq+s*s*aqq;
			double new_aqq=s*s*app+2*s*c*apq+c*c*aqq;
	...
	}while(largestupperoffdiag(A)>threshold); //while(changed!=0);
	// update lower triangular part
	double Aji;
	for(int i = 0; i<n; i++) {
		for (int j = 0; j<i; j++) {
			Aji = gsl_matrix_get(A,j,i);
